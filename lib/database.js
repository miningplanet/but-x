...
module.exports = {
  ...
  get_latest_coinbase_tx: function(tx_type, cb, net=settings.getDefaultNet()) {
  ...
  // for later use
  count_blocks_by_algorithm: function(height=0, cb, net=settings.getDefaultNet()) {
    BlockDb[net].aggregate([{"$match": {"height": {"$gt":height}}}, {"$group" : {_id: "$algo", "count": {"$sum":1}}}, {"$sort": {"_id": 1}}]).then(algos => {
      return cb(algos)
    }).catch((err) => {
      console.error("Failed to count blocks by algorithm for chain '%s': %s", hash, net, err)
      return cb(err)
    })
  },
  ...
  count_addresses: function(cb, net=settings.getDefaultNet()) {
  ...
  count_utxos: function(cb, net=settings.getDefaultNet()) {
  ...
  count_tx_by_type: function(height=0, cb, net=settings.getDefaultNet()) {
    TxDb[net].aggregate([{"$match": {"blockindex": {"$gt":height}}}, {"$group" : {_id: "$tx_type", "count": {"$sum":1}}}, {"$sort": {"_id": 1}}]).then(txtypes => {
      const tx_types = settings.get(net, 'tx_types')
      for (i = 0; i < tx_types.length; i++) {
        var found = false
        for (j = 0; j < txtypes.length; j++) {
          if (tx_types[i] == txtypes._id) {
            found = true
            break
          }
        }
        if (!found) {
          txtypes.push({ "_id" : i, "count" : 0 })
        }
      }
      return cb(txtypes)
    }).catch((err) => {
      console.error("Failed to count tx types by blocks for chain '%s': %s", hash, net, err)
      return cb(err)
    })
  },

  get_last_txs_ajax: function(start, length, min, type, cb, net=settings.getDefaultNet()) {
    ...
    var findCriteria = {'total': {$gte: min}, 'vout': { $gte: { $size: 1 }}}
    if (!isNaN(type) && type > -1) {
      findCriteria = {'total': {$gte: min}, 'vout': { $gte: { $size: 1 }}, 'tx_type': type}
    }
  ...
  create_market: function(coin_symbol, pair_symbol, reverse, market, ext_market_url, referal, logo, cb, net=settings.getDefaultNet()) {
  ...
  check_market: function(market, coin_symbol, pair_symbol, reverse, cb, net=settings.getDefaultNet()) {
  ...
  // gets orders sum aggregation for a given trading pair over the available markets.
  get_order_aggregation: function(coin_symbol, type, cb, net=settings.getDefaultNet()) {
  ...
  // creates initial richlist entry in database; called on first launch of but-x + after restore or delete database
  ...
  update_markets_db: function(market, coin_symbol, pair_symbol, reverse, cb, net=settings.getDefaultNet()) {
    const coin = settings.getCoin(net)
    if (fs.existsSync('./lib/markets/' + market + '.js')) {
