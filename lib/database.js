        ...
        db.peers.upstream.forEach(function (url) {
          ...
          socket.addEventListener('message', (m) => {
            ...
            if (m.data == Peers.UPSTREAM_GET_PEERS + net) {
              ...
              module.exports.get_buy_order_aggregation(params[2], params[3], params[4], params[5], function(history) {      
                socket.send(JSON.stringify({ "event": m.data, "data": history }))
              }, net)
            } else if (m.data.startsWith(Peers.UPSTREAM_GET_SELL_ORDER_AGGR + net)) {
              module.exports.get_sell_order_aggregation(params[2], params[3], params[4], params[5], function(history) {      
                socket.send(JSON.stringify({ "event": m.data, "data": history }))
              }, net)
            }
