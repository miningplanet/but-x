...
const { MessageEvent } = require('express-ws')
...
const dbindexCache              = new TTLCache({ max: wlength, ttl: settings.cache.dbindex * 1000, updateAgeOnGet: false, noUpdateTTL: false })
const addressCache              = new TTLCache({ max: wlength, ttl: settings.cache.addresses * 1000, updateAgeOnGet: false, noUpdateTTL: false })
const addressTxCache            = new TTLCache({ max: wlength, ttl: settings.cache.addresstxes * 1000, updateAgeOnGet: false, noUpdateTTL: false })
...
const richlistCache             = new TTLCache({ max: wlength, ttl: settings.cache.richlist * 1000, updateAgeOnGet: false, noUpdateTTL: false })
const txsCache                  = new TTLCache({ max: wlength * 2, ttl: settings.cache.txs * 1000, updateAgeOnGet: false, noUpdateTTL: false })
...
function has_upstream_peers() {
  return upstreamPeers.length > 0
}

function get_upstream_peer() {
  return upstreamPeers[upstreamPeers.length -1]
}
...
module.exports = {
      ...
      if (db && db.peers && db.peers.enabled == true && Array.isArray(db.peers.upstream)) {
        ...
        db.peers.upstream.forEach(function (url) {
          ...
          socket.addEventListener('message', (m) => {
            debugPeers("Received event: %o", m.data)
            ...
            if (m.data == Peers.UPSTREAM_GET_PEERS + net) {
              ...
              const hash = m.data.replace(Peers.UPSTREAM_GET_ADDRESS + net + '/', '') 
              ...
              const params = m.data.replace(Peers.UPSTREAM_GET_ADDRESS_TXES + net + '/', '').split('/')
              const hash = params[0]
              const start = params[1]
              const length = params[2]
              debugPeers("%s params: %s, %s, %d, %d", Peers.UPSTREAM_GET_ADDRESS_TXES, net, hash, start, length)

              module.exports.get_address_txs(hash, start, length, function(data) {      
                // TODO: Fix peer data. Balance is undefined
                const obj = {}
                obj.event = m.data
                obj.data = data // { "data": [...], "recordsTotal": count, "recordsFiltered": count }
                socket.send(JSON.stringify(obj))
              }, net)
            } else if (m.data.startsWith(Peers.UPSTREAM_GET_TXES_BY_BLOCKHASH + net)) {
              const params = m.data.replace(Peers.UPSTREAM_GET_TXES_BY_BLOCKHASH + net + '/', '').split('/')
              const hash = params[0]
              module.exports.find_txs_by_blockhash(hash, function(txs) {      
                const obj = {}
                obj.event = m.data
                obj.data = txs
                socket.send(JSON.stringify(obj))
          ...
          if (push == true)
            module.exports.push_upstream_peer_client(socket)
        })
      }
  ...
  push_upstream_peer_server: function(peer) { // net=settings.getDefaultNet(), 
    debugPeers("Pushed upstream server: %o", peer._sender._socket._peername)
    upstreamPeers.push(peer)
  },

  update_upstream_peer_servers: function(wsInstance) { // net=settings.getDefaultNet(), 
    ...
    for(let i=0; i < clientsSet.size; i++) {
      ...
      debugPeers("Alive upstream peer %o.", peer._sender._socket._peername)
      newUpstreamPeers.push(peer)
    }
    upstreamPeers = newUpstreamPeers
  },

  push_upstream_peer_client: function(peer) { // net=settings.getDefaultNet(), 
    debugPeers("Pushed upstream client: %o", peer._url)
    upstreamPeers.push(peer)
  },
  ...
  find_txs_by_blockhash: function(hash, cb, net=settings.getDefaultNet()) {
    ...
    if (settings.needsUpstream(net)) {
      ...
      get_txs_by_blockhash_local(cb, hash, net)
    }
  },
  ...
  get_dbindex: function(coinName, cb, net=settings.getDefaultNet()) {
    ...
    if (settings.needsUpstream(net)) {
      if (has_upstream_peers()) {
        ...
        get_upstream_peer().send(Peers.UPSTREAM_GET_DBINDEX + net, function(m) {
          wait_until_upstream_cached(cb, dbindexCache, net)
        })
      } else {
        cb(WAIT_FOR_UPSTREAM_PEERS)
      }
  ...
  get_address: function(hash, cb, net=settings.getDefaultNet()) {
    ...
    if (settings.needsUpstream(net)) {
      if (has_upstream_peers()) {
        debugPeers("Get remote address for net '%s', number of peers %d", net, upstreamPeers.length)
        get_upstream_peer().send(Peers.UPSTREAM_GET_ADDRESS + net + '/' + hash, function(m) {
          wait_until_upstream_cached(cb, addressCache, net)
        })
      } else {
        cb(WAIT_FOR_UPSTREAM_PEERS)
      }
  ...
  get_richlist: function(coin, cb, net=settings.getDefaultNet()) {
    ...
    if (settings.needsUpstream(net)) {
      if (has_upstream_peers()) {
        ...
        get_upstream_peer().send(Peers.UPSTREAM_GET_RICHLIST + net, function(m) {
          wait_until_upstream_cached(cb, richlistCache, net)
        })
      } else {
        cb(WAIT_FOR_UPSTREAM_PEERS)
      }
  ...
  get_address_txs: function(hash, start, length, cb, net=settings.getDefaultNet()) {
    const r = addressTxCache.get(net)
    if (r) {
      cb(r)
      return r
    }
    ...
    if (settings.needsUpstream(net)) {
      if (has_upstream_peers()) {
        ...
        get_upstream_peer().send(Peers.UPSTREAM_GET_ADDRESS_TXES + net + '/' + hash + '/' + start + '/' + length, function(m) { //  + '/internal'
          wait_until_upstream_cached(cb, addressTxCache, net)
        })
      } else {
        cb(WAIT_FOR_UPSTREAM_PEERS)
      }
  ...
  get_peers: function(cb, net=settings.getDefaultNet()) {
    ...
    if (settings.needsUpstream(net)) {
      if (has_upstream_peers()) {
        ...
        get_upstream_peer().send(Peers.UPSTREAM_GET_PEERS + net, function(m) {
          wait_until_upstream_cached(cb, peersCache, net)
        })
      } else {
        cb(WAIT_FOR_UPSTREAM_PEERS)
      }
  ...
  get_masternodes: function (cb, net=settings.getDefaultNet()) {
    ...
    if (settings.needsUpstream(net)) {
      if (has_upstream_peers()) {
        ...
        get_upstream_peer().send(Peers.UPSTREAM_GET_MASTERNODES + net, function(m) {
          wait_until_upstream_cached(cb, masternodesCache, net)
        })
      } else {
        cb(WAIT_FOR_UPSTREAM_PEERS)
      }
