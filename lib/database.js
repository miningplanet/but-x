...
function init_markets(cb, net=settings.getDefaultNet()) {
...
module.exports = {
  ...
  update_richlist: function(list, cb, net=settings.getDefaultNet()) {
    ...
    if (list == 'received') {
      ...
      AddressDb[net].find({a_id: { $nin: burn_addresses }}, 'a_id name balance received').sort({received: 'desc'}).limit(total_addresses).exec().then((addresses) => {
        RichlistDb[net].updateOne({coin: coin.name}, {
          received: addresses
        }).then(() => {
          return cb()
      ...
      if (richlist_page.burned_coins.addresses == null || richlist_page.burned_coins.addresses.length == 0 || !richlist_page.burned_coins.include_burned_coins_in_distribution) {
        ...
        AddressDb[net].find({a_id: { $nin: burn_addresses }}, 'a_id name balance received').sort({balance: 'desc'}).limit(total_addresses).exec().then((addresses) => {
          RichlistDb[net].updateOne({coin: coin.name}, {
            balance: addresses
          }).then(() => {
            return cb();
        ...
        AddressDb[net].find({}, 'a_id name balance received').sort({balance: 'desc'}).limit(total_addresses + burn_addresses.length).exec().then((addresses) => {
  ...
  create_market: function(coin_symbol, pair_symbol, market, ext_market_url, referal, cb, net=settings.getDefaultNet()) {
  ...
  get_markets_summary: function(cb, net=settings.getDefaultNet()) {
    MarketsDb[net].find({}, { _id: 0, market: 1, referal: 1, coin_symbol: 1, pair_symbol: 1, summary: 1, ext_market_url: 1, referal: 1 }).then((data) => {
      return cb(data ? data : null)
    }).catch((err) => {
  ...
  get_markets: function(cb, net=settings.getDefaultNet()) {
    MarketsDb[net].find({}, { _id: 0, market: 1, coin_symbol: 1, pair_symbol: 1, summary: 1, chartdata: 1, buys: 1, sells: 1, history: 1, ext_market_url: 1, referal: 1 }).then((data) => {
      return cb(data ? data : null)
    }).catch((err) => {
  ...
  delete_richlist: function(coin, cb, net=settings.getDefaultNet()) {
    RichlistDb[net].findOneAndRemove({coin: coin}).then((data) => {
  ...
  update_network_history: function(height, cb, net=settings.getDefaultNet()) {
  ...
  check_masternodes_and_upgrade_schema: function(cb, net=settings.getDefaultNet()) {
    MasternodeDb[net].count({}).then((count) => {
      return cb(count)
    }).catch((err) => {
  ...
  count_masternodes: function (cb, net=settings.getDefaultNet()) {
    MasternodeDb[net].count({}).then((masternodes) => {
      MasternodeDb[net].count({ status: 'ENABLED'}).then((enabled) => {
        r = {}
        r.count = masternodes
        r.active = enabled
        return cb(r)
  ...
  check_networkhistory_and_upgrade_schema: function(cb, net=settings.getDefaultNet()) {
    NetworkHistoryDb[net].count({}).then((count) => {
      return cb(count)
    }).catch((err) => {
