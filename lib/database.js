...
module.exports = {
  ...
  create_market: function(coin_symbol, pair_symbol, market, ext_market_url, referal, cb) {
  ...
  get_markets_summary: function(cb) {
    Markets.find({}, { _id: 0, market: 1, referal: 1, coin_symbol: 1, pair_symbol: 1, summary: 1, ext_market_url: 1, referal: 1 }, function(err, data) {
      if (data)
        return cb(data);
      else
        return cb(null);
  ...
  get_markets: function(cb) {
    Markets.find({}, { _id: 0, market: 1, coin_symbol: 1, pair_symbol: 1, summary: 1, chartdata: 1, buys: 1, sells: 1, history: 1, ext_market_url: 1, referal: 1 }, function(err, data) {
      if (data)
        return cb(data);
      else
        return cb(null);
  ...
  // get the list of masternodes from local collection
  count_masternodes: function (cb) {
    Masternode.count({}, function (err, masternodes) {
      if (err)
        return cb({});
      else {
        Masternode.count({ status: 'ENABLED'}, function (err, enabled) {
          if (err)
            return cb({});
          else 
            r = {}
