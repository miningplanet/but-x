...
module.exports = {
      ...
      if (db && db.peers && db.peers.enabled == true && Array.isArray(db.peers.upstream)) {
        ...
        db.peers.upstream.forEach(function (url) {
          ...
          socket.onmessage = (event) => {
            // TODO: Comes after peer received event:
          ...
          socket.addEventListener('message', (m) => {
            console.log("Received event: %o", m.data)
            ...
            if (m.data == Peers.UPSTREAM_GET_PEERS + net) {
              ...
              module.exports.get_stats(coinName, function(stats) {      
                const obj = {}
                obj.event = m.data
                obj.data = stats
                socket.send(JSON.stringify(obj))
            ...
            // setTimeout(function() {
            //   socket.send('poll_upstream_queue')
            // }, 1000)
          })
  ...
  get_stats: function(coinName, cb, net=settings.getDefaultNet()) {
    ...
    if (settings.hasUpstream(net, upstreamPeers)) {
      debug("Get remote coinstats for net '%s', number of peers %d", net, upstreamPeers.length)
      upstreamPeers[0].send(Peers.UPSTREAM_GET_COINSTATS + net, function(m) {
        module.exports.wait_until_upstream_cached(cb, statsCache, net)
      })
    } else {
      module.exports.get_stats_local(coinName, cb, net)
    }
  },

  get_stats_local: function(coinName, cb, net=settings.getDefaultNet()) {
    StatsDb[net].findOne({coin: coinName}).then((data) => {
      return cb(data)
    }).catch((err) => {
  ...
  get_dbindex: function(coinName, cb, net=settings.getDefaultNet()) {
    ...
    if (settings.hasUpstream(net, upstreamPeers)) {
      debug("Get remote db index for net '%s', number of peers %d", net, upstreamPeers.length)
      upstreamPeers[0].send(Peers.UPSTREAM_GET_DBINDEX + net, function(m) {
        module.exports.wait_until_upstream_cached(cb, dbindexCache, net)
      })
    } else {
      module.exports.get_dbindex_local(coinName, cb, net)
    }
  },

  get_dbindex_local: function(coinName, cb, net=settings.getDefaultNet()) {
  ...
  get_address: function(hash, cb, net=settings.getDefaultNet()) {
    ...
    if (settings.hasUpstream(net, upstreamPeers)) {
      debug("Get remote address '%s' for net '%s', number of peers %d", hash, net, upstreamPeers.length)
      upstreamPeers[0].send(Peers.UPSTREAM_GET_ADDRESS + net + '/' + hash, function(m) {
        module.exports.wait_until_upstream_cached(cb, addressCache, net)
      })
    } else {
      module.exports.get_address_local(hash, cb, net)
    }
  },

  get_address_local(hash, cb, net=settings.getDefaultNet()) {
  ...
  get_richlist: function(coin, cb, net=settings.getDefaultNet()) {
    ...
    if (settings.hasUpstream(net, upstreamPeers)) {
      debug("Get remote richlist for net '%s', number of peers %d", net, upstreamPeers.length)
      upstreamPeers[0].send(Peers.UPSTREAM_GET_RICHLIST + net, function(m) {
        module.exports.wait_until_upstream_cached(cb, richlistCache, net)
      })
    } else {
      module.exports.get_richlist_local(coin, cb, net)
    }
  },

  get_richlist_local(coin, cb, net=settings.getDefaultNet()) {
    RichlistDb[net].findOne({coin: coin}).then((richlist) => {
      if (richlist)
        return cb(richlist)
      else
  ...
  get_peers: function(cb, net=settings.getDefaultNet()) {
    ...
    if (settings.hasUpstream(net, upstreamPeers)) {
      debug("Get remote network peers for net '%s', number of peers %d", net, upstreamPeers.length)
      upstreamPeers[0].send(Peers.UPSTREAM_GET_PEERS + net, function(m) {
        module.exports.wait_until_upstream_cached(cb, peersCache, net)
      })
    } else {
      module.exports.get_peers_local(cb, net)
    }
  },

  get_peers_local: function(cb, net=settings.getDefaultNet()) {
  ...
  get_masternodes: function (cb, net=settings.getDefaultNet()) {
    ...
    if (settings.hasUpstream(net, upstreamPeers)) {
      debug("Get remote network masternodes for net '%s', number of peers %d", net, upstreamPeers.length)
      upstreamPeers[0].send(Peers.UPSTREAM_GET_MASTERNODES + net, function(m) {
        module.exports.wait_until_upstream_cached(cb, masternodesCache, net)
      })
    } else {
  ...
  // get the list of masternodes
  get_masternodes_local: function (cb, net=settings.getDefaultNet()) {
  ...
  wait_until_upstream_cached: async function wait_until_upstream_cached(cb, cache, net) {
