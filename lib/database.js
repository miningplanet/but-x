...
function get_stats(coinName, cb, net=settings.getDefaultNet()) {
  // Return to default if not found.
...
function get_address_local(hash, cb, net=settings.getDefaultNet()) {
  AddressDb[net].findOne({a_id: hash}).lean().then((address) => {
    if (address)
      return cb(address)
    else
      return cb()
...
function get_asset_index_by_name_local(name, cb, net=settings.getDefaultNet()) {
  AssetsDb[net].findOne({ name: name }).then((asset) => {
    cb(asset ? asset : null)
  }).catch((err) => {
    console.error("Failed to find asset index '%s' for chain '%s': %s", name, net, err)
    cb(null)
  })
}

function get_assets_local(start, length, cb, net=settings.getDefaultNet()) {
  AssetsDb[net].find({}).sort({ height: 1 }).skip(Number(start)).limit(Number(length)).then((assets) => {
    cb(assets ? assets : null)
  }).catch((err) => {
    console.error("Failed to get assets for chain '%s': %s", net, err)
    cb(null)
  })
}

function get_asset_by_name_local(name, cb, net=settings.getDefaultNet()) {
  AssetsDb[net].findOne({name: name}).then((asset) => {
    cb(asset ? asset : null)
  }).catch((err) => {
    console.error("Failed to get asset with name '%s' for chain '%s': %s", name, net, err)
    cb(null)
  })
}

function get_ownership_asset_from_tx_by_name_local(name, cb, net=settings.getDefaultNet()) {
  name = name + '!'
  TxDb[net].findOne({ tx_type: '8', 'vout.name': name }).then((tx) => {
    cb(tx ? tx : null)
  }).catch((err) => {
    console.error("Failed to find asset '%s' for chain '%s': %s", name, net, err)
    cb(null)
  })
}

function get_latest_asset_tx_by_name_local(name, start, length, cb, net=settings.getDefaultNet()) {
  TxDb[net].find({ tx_type: {$ne : '8'}, 'vout.name': name }).sort({timestamp: -1}).skip(Number(start)).limit(Number(length)).then((txs) => {
    cb(txs ? txs : null)
  }).catch((err) => {
    console.error("Failed to find latest transactions for asset '%s' for chain '%s': %s", name, net, err)
    cb(null)
  })
}
      ...
      AddressTxDb[net].aggregate([
        ...
        AddressTxDb[net].find({a_id: hash}).sort({blockindex: -1}).skip(Number(start)).limit(Number(length)).exec().then((address_tx) => {
          ...
          lib.syncLoop(count, function (loop) {
            ...
            return cb({ "data": txs, "recordsTotal": totalCount, "recordsFiltered": totalCount })
          })
...
function get_last_txs_local(cb, stats, start, length, findCriteria, net=settings.getDefaultNet()) {
  TxDb[net].find(findCriteria).sort({blockindex: -1}).skip(Number(start)).limit(Number(length)).exec().then((txs) => {
    ...
    return cb(data, stats ? stats.count_txes : 0)
  }).catch((err) => {
...
function create_user_local(uuid, address, pwd, salt, balance, cb, net) {
  UsersDb[net].create({
    uuid: uuid,
    address: address,
    net: net,
...
function get_user_local(address, cb, net) {
  UsersDb[net].findOne({ address: address }).exec().then((user) => {
    cb(data)
  }).catch((err) => {
    console.error("Failed to find user '%s' for chain '%s': %s", address, net, err)
    cb(null)
  })
}
...
module.exports = {
  ...
  connection_factory: function(net, initpeers, dbstring, cb) {
    ...
    AssetsDb[net] = con.model('Asset', require('../models/asset'))
    ...
    UsersDb[net] = con.model('Users', require('../models/users'))
    XPeersDb[net] = con.model('Xpeers', require('../models/xpeers'))

    // TODO: Peers. Not the best place here, but lets do it for a while.
    if (initpeers == true) {
      ...
      if (db && db.peers && db.peers.enabled == true && Array.isArray(db.peers.upstream)) {
        ...
        db.peers.upstream.forEach(function (url) {
          ...
          socket.onmessage = (event) => {
            // TODO: Peers: Comes before 'Received event:'
          ...
          socket.addEventListener('message', (m) => {
            ...
            if (m.data == Peers.UPSTREAM_GET_PEERS + net) {
              ...
              module.exports.get_address_txs(params[2], params[3], params[4], function(data) {      
                // TODO: Peers: Balance is undefined
  ...
  // TODO: Peers: Impl. update_richlist_claim_name
  ...
  // TODO: Peers: Impl. update_masternode_claim_name
...
get_last_txs: function(start, length, min, type, cb, net=settings.getDefaultNet()) {
    ...
    check_stats(coin.name, function(stats_exists) {
      ...
      create_stats(coin.name, skip, function() {
        ...
        module.exports.get_stats(coin.name, function(stats) {
          console.log('+ %s DB initialization complete. Block stats height %d', net, stats ? stats.count : -1)
          return cb(stats)
        }, net)
      }, net)
    }, net)
  ...
  get_stats: get_stats,
  get_stats_local: get_stats_local,
  get_address_local: get_address_local,
  get_markets_local: get_markets_local,
  get_market_data: get_market_data,
  get_asset_index_by_name_local: get_asset_index_by_name_local,
  get_assets_local: get_assets_local,
  get_asset_by_name_local: get_asset_by_name_local,
  get_ownership_asset_from_tx_by_name_local: get_ownership_asset_from_tx_by_name_local,
  get_latest_asset_tx_by_name_local: get_latest_asset_tx_by_name_local,
