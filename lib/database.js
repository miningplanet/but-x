...
function get_latest_asset_tx_by_name_local(name, cb, net=settings.getDefaultNet()) {
  TxDb[net].find({ tx_type: {$ne : '8'}, 'vout.name': name }).sort({timestamp: -1}).limit(100).then((txs) => {
    cb(txs ? txs : null)
  }).catch((err) => {
