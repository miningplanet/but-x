...
module.exports = {
  ...
  get_data: function(settings, cb) {
    const coin = settings.getCoin(settings.net)
    var error = null;
    get_pair_id(coin.toUpperCase(), settings.exchange.toUpperCase(), function (err, stex_id) {
      get_chartdata(coin.toUpperCase(), settings.exchange.toUpperCase(), stex_id, function (err, chartdata) {
        if (err) { chartdata = []; error = err; }
        get_orders(coin.toUpperCase(), settings.exchange.toUpperCase(), stex_id, function (err, buys, sells) {
          if (err) { error = err; }
          get_trades(coin.toUpperCase(), settings.exchange.toUpperCase(), stex_id, function (err, trades) {
            if (err) { error = err; }
            get_summary(coin.toUpperCase(), settings.exchange.toUpperCase(), stex_id, function (err, stats) {
              if (err) { error = err; }
