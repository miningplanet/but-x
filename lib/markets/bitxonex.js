...
var base_url = 'https://www.bitxonex.com/api/v2/trade/public/markets/';
...
module.exports = {
  ...
  get_data: function(settings, cb) {
    ...
    var error = null;
    get_chartdata(coin, settings.exchange, function (err, chartdata) {
      if (err) { chartdata = []; error = err; }
      get_orders(coin, settings.exchange, function(err, buys, sells) {
        if (err) { error = err; }
        get_trades(coin, settings.exchange, function(err, trades) {
          if (err) { error = err; }
          get_summary(coin, settings.exchange, function(err, stats) {
            if (err) { error = err; }
