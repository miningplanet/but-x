...
module.exports = {
  ...
  get_data: function(settings, cb) {
    ...
    var error = null;
    get_chartdata(coin.toLowerCase(), settings.exchange.toLowerCase(), function (err, chartdata) {
      if (err) { chartdata = []; error = err; }
      get_orders(coin.toLowerCase(), settings.exchange.toLowerCase(), function(err, buys, sells) {
        if (err) { error = err; }
        get_trades(coin.toLowerCase(), settings.exchange.toLowerCase(), function(err, trades) {
          if (err) { error = err; }
          get_summary(coin.toLowerCase(), settings.exchange.toLowerCase(), function(err, stats) {
            if (err) { error = err; }
