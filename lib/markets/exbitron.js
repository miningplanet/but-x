...
function get_summary(coin, exchange, buys, reverse, cb) {
...
function get_trades(coin, exchange, cb) {
  const req_url = "https://api.exbitron.com/api/v1/cmc/" + coin.toLowerCase() + exchange.toLowerCase() + '/trades?limit=50&order_by=desc';
...
module.exports = {
  ...
  get_data: function(settings, cb) {
    var error = null
    // TODO: There seems to be not trade list available.
    // get_chartdata(settings.coin, settings.exchange, function (err, chartdata) {
      ...
      get_orders(settings.coin, settings.exchange, function(err, buys, sells) {
        ...
        // get_trades(settings.coin, settings.exchange, function(err, trades) {
          if (err) { error = err }
          get_summary(settings.coin, settings.exchange, buys, settings.reverse, function(err, stats) {
            if (err) { error = err }
            return cb(error, {buys: buys, sells: sells, chartdata: [], trades: [], stats: stats}) // chartdata, trades
          })
        // })
      })
    // })
