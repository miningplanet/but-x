...
module.exports = {
  ...
  get_data: function(settings, cb) {
    ...
    var error = null;
    get_chartdata(coin.toUpperCase(), settings.exchange.toUpperCase(), function (err, chartdata) {
      if (err) { chartdata = []; error = err; }
      get_orders(coin.toUpperCase(), settings.exchange.toUpperCase(), function(err, buys, sells) {
        if (err) { error = err; }
        get_trades(coin.toUpperCase(), settings.exchange.toUpperCase(), function(err, trades) {
          if (err) { error = err; }
          get_summary(coin.toUpperCase(), settings.exchange.toUpperCase(), (buys == null || buys.length == 0 ? null : buys[0].price), (sells == null || sells.length == 0 ? null : sells[0].price), function(err, stats) {
            if (err) { error = err; }
