...
function convertHashUnits(hashes, net='mainnet') {
...
function encodeP2PKaddress(p2pk_descriptor, cb, net='mainnet') {
...
module.exports = {
  ...
  get_hashrate: function(cb, net='mainnet') {
    const shared_pages = settings.get(net, 'shared_pages')
    ...
    if (shared_pages.page_header.panels.network_panel.nethash == 'netmhashps') {
      ...
      var cmd = prepareRpcCommand(api_cmds.getmininginfo);
      ...
      if (!(cmd.method == '' && cmd.parameters.length == 0)) {
        ...
        if (api_cmds.use_rpc) {
          ...
          rpcCommand([{net:net, method:cmd.method, parameters: cmd.parameters}], function(response) {
            ...
            var hashps = {};
            if (response.networkhashps_ghostrider) {
              hashps.nethash_ghostrider = convertHashUnits(response.networkhashps_ghostrider, net);
            }
            if (response.networkhashps_yespower) {
              hashps.nethash_yespower = convertHashUnits(response.networkhashps_yespower, net);
            }
            if (response.networkhashps_lyra2) {
              hashps.nethash_lyra2 = convertHashUnits(response.networkhashps_lyra2, net);
            }
            if (response.networkhashps_sha256d) {
              hashps.nethash_sha256d = convertHashUnits(response.networkhashps_sha256d, net);
            }
            if (response.networkhashps_scrypt) {
              hashps.nethash_scrypt = convertHashUnits(response.networkhashps_scrypt, net);
            }
            if (response.networkhashps_butkscrypt) {
              hashps.nethash_butkscrypt = convertHashUnits(response.networkhashps_butkscrypt, net);
            }
          ...
          var uri = base_url + 'getmininginfo/' + net
          request({uri: uri, json: true}, function (error, response, body) {
            ...
            if (body == 'There was an error. Check your console.') {
              ...
              var hashps = {};
              if (body.networkhashps_ghostrider) {
                hashps.nethash_ghostrider = convertHashUnits(body.networkhashps_ghostrider, net);
              }
              if (body.networkhashps_yespower) {
                hashps.nethash_yespower = convertHashUnits(body.networkhashps_yespower, net);
              }
              if (body.networkhashps_lyra2) {
                hashps.nethash_lyra2 = convertHashUnits(body.networkhashps_lyra2, net);
              }
              if (body.networkhashps_sha256d) {
                hashps.nethash_sha256d = convertHashUnits(body.networkhashps_sha256d, net);
              }
              if (body.networkhashps_scrypt) {
                hashps.nethash_scrypt = convertHashUnits(body.networkhashps_scrypt, net);
              }
              if (body.networkhashps_butkscrypt) {
                hashps.nethash_butkscrypt = convertHashUnits(body.networkhashps_butkscrypt, net);
              }
  ...
  get_difficulty: function(net, cb) {
    ...
    const api_cmds = settings.get(net, 'api_cmds')
    var cmd = prepareRpcCommand(api_cmds.getdifficulty);
      ...
      if (api_cmds.use_rpc) {
        rpcCommand([{net:net, method: 'getblockchaininfo', parameters: cmd.parameters}], function(response) {
          ...
          if (response == 'There was an error. Check your console.')
            ...
            obj.pow_default_algo = isButkoin ? 'butkscrypt' : 'ghostrider';
        ...
        var uri = base_url + 'getdifficulty/' + net;
        request({uri: uri, json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == 'There was an error. Check your console.')
            return cb(null);
  ...
  get_masternodecount: function(net, cb) {
    const api_cmds = settings.get(net, 'api_cmds')
    var cmd = prepareRpcCommand(api_cmds.getmasternodecount);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (api_cmds.use_rpc) {
        ...
        var uri = base_url + 'getmasternodecount/' + net
        request({uri: uri, json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == 'There was an error. Check your console.')
            return cb(null);
  ...
  get_blockcount: function(cb, net='mainnet') {
    const api_cmds = settings.get(net, 'api_cmds')
  ...
  get_blockhash: function(height, cb, net='mainnet') {
    const api_cmds = settings.get(net, 'api_cmds')
  ...
  get_block: function(hash, cb, net='mainnet') {
    const api_cmds = settings.get(net, 'api_cmds')
  ...
  get_rawtransaction: function(hash, cb, net='mainnet') {
    const api_cmds = settings.get(net, 'api_cmds')
  ...
  // only used by heavy
  get_maxmoney: function(cb, net='mainnet') {
  ...
  get_maxvote: function(cb, net='mainnet') {
  ...
  get_vote: function(cb, net='mainnet') {
  ...
  get_phase: function(cb, net='mainnet') {
  ...
  get_reward: function(cb, net='mainnet') {
  ...
  get_estnext: function(cb, net='mainnet') {
  ...
  get_nextin: function(cb, net='mainnet') {
  ...
  get_descriptorinfo: function(descriptor, cb, net='mainnet') {
  ...
  get_deriveaddresses: function(descriptor, cb, net='mainnet') {
  ...
  balance_supply: function(cb, net='mainnet') {
    AddressDb[net].find({}, 'balance').where('balance').gt(0).exec(function(err, docs) {
      var count = 0;
    ...
    if (settings.sync.supply == 'HEAVY') {
      ...
      if (!(cmd.method == '' && cmd.parameters.length == 0)) {
        if (api_cmds.use_rpc) {
          ...
          var uri = base_url + 'getsupply/' + net
          request({uri: uri, json: true}, function (error, response, body) {
            // check if an error msg was received from the web api server
            if (body == 'There was an error. Check your console.')
              return cb(null);
      ...
      const api_cmds = settings.get(net, 'api_cmds')
      var cmd = prepareRpcCommand(api_cmds.getinfo);
      ...
      const api_cmds = settings.get(net, 'api_cmds')
      var cmd = prepareRpcCommand(api_cmds.gettxoutsetinfo);
      ...
      if (!(cmd.method == '' && cmd.parameters.length == 0)) {
        if (api_cmds.use_rpc) {
          ...
          var uri = base_url + 'gettxoutsetinfo/' + net
          request({uri: uri, json: true}, function (error, response, body) {
            // check if an error msg was received from the web api server
  ...
  get_peerinfo: function(net='mainnet', cb) {
    const api_cmds = settings.get(net, 'api_cmds')
  ...
  verify_message: function(net='mainnet', address, signature, message, cb) {
    const api_cmds = settings.get(net, 'api_cmds')
  ...
  prepare_vout: function(net='mainnet', vout, txid, vin, vhidden, cb) {
    var arr_vout = [];
    var arr_vin = vin;
    var tx_type = null;
  ...
  get_input_addresses: function(net='mainnet', input, vout, cb) {
    var addresses = [];
  ...
  prepare_vin: function(net='mainnet', tx, cb) {
    var arr_vin = [];
    var tx_type = null;
  ...
  create_lock: function(lock, net='mainnet') {
    const fs = require('fs');
  ...
  remove_lock: function(lock, net='mainnet') {
    const fs = require('fs');
    const fname = './tmp/' + net + '-' + lock + '.pid';
    try {
      fs.unlinkSync(fname);
  ...
  is_locked: function(lock_array, net='mainnet') {
    const fs = require('fs');
