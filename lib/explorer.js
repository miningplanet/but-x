        ...
        if (api_cmds.use_rpc) {
          ...
          rpcCommand([{net:net, method:cmd.method, parameters: cmd.parameters}], function(response) {
            ...
            if (response.networkhashps_ghostrider) {
              hashps.nethash_ghostrider = convertHashUnits(response.networkhashps_ghostrider);
            }
            if (response.networkhashps_yespower) {
              hashps.nethash_yespower = convertHashUnits(response.networkhashps_yespower);
            }
            if (response.networkhashps_lyra2) {
              hashps.nethash_lyra2 = convertHashUnits(response.networkhashps_lyra2);
            }
            if (response.networkhashps_sha256d) {
              hashps.nethash_sha256d = convertHashUnits(response.networkhashps_sha256d);
            }
            if (response.networkhashps_scrypt) {
              hashps.nethash_scrypt = convertHashUnits(response.networkhashps_scrypt);
            }
            if (response.networkhashps_butkscrypt) {
              hashps.nethash_butkscrypt = convertHashUnits(response.networkhashps_butkscrypt);
            }
          ...
          request({uri: uri, json: true}, function (error, response, body) {
            ...
            if (body == 'There was an error. Check your console.') {
              ...
              if (body.networkhashps_ghostrider) {
                hashps.nethash_ghostrider = convertHashUnits(body.networkhashps_ghostrider);
              }
              if (body.networkhashps_yespower) {
                hashps.nethash_yespower = convertHashUnits(body.networkhashps_yespower);
              }
              if (body.networkhashps_lyra2) {
                hashps.nethash_lyra2 = convertHashUnits(body.networkhashps_lyra2);
              }
              if (body.networkhashps_sha256d) {
                hashps.nethash_sha256d = convertHashUnits(body.networkhashps_sha256d);
              }
              if (body.networkhashps_scrypt) {
                hashps.nethash_scrypt = convertHashUnits(body.networkhashps_scrypt);
              }
              if (body.networkhashps_butkscrypt) {
                hashps.nethash_butkscrypt = convertHashUnits(body.networkhashps_butkscrypt);
              }
