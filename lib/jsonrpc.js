...
Client.prototype.call = function(method, params, callback, errback, path) {
  ...
  if (Array.isArray(method)) {
    ...
    method.forEach(function(batchCall, i) {
      ...
      if (batchCall.method.indexOf("/") > -1) {
        ...
        batchCall.net = net;
        console.log("Got net: %s", batchCall.net);
      }
      requestJSON.push({
        id: time + '-' + i,
        net: batchCall.net,
  ...
  console.log("Request options '%s'.", JSON.stringify(requestOptions));
  ...
  request.on('response', function(response) {
    ...
    response.on('end', function() {
      ...
      try {
        ...
        console.log("Decoded response '%s'.", JSON.stringify(decoded));
      } catch (e) {
  ...
  // .replace(/\\/g, "")
  console.debug("Request JSON '%s'.", JSON.stringify(requestJSON));
