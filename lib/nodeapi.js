...
const METHOD_RESTRICTED = 'This method is restricted.'
...
const RESTRICT_LIST_DEFAULT_SAFE = [
  'dumpprivkey', 
  'walletpassphrasechange', 
  'stop']

const RESTRICT_LIST_READ_ONLY = [
  'addmultisigaddress', 
  'addnode', 
  'backupwallet', 
  'createmultisig', 
  ...
  'sendfrom', 
  'sendmany', 
  'sendrawtransaction', // TODO: Impl. sendrawtransaction
  'sendtoaddress', 
  'setaccount', 
  'setgenerate', 
  'settxfee', 
...
module.exports = function() {
  function express_app() {
            ...
            if (err) {
              ...
              if (typeof response === 'object') {
                if (method == 'getdifficulty') {
                  // TODO: Re-factor. Only called if the network history is created initially. See sync_chain.js update_network_history_db.
    ...
    function hasAccess(req, res, next) {
      ...
      if (settings.api_page.public_apis.rpc[method] != null) {
        ...
        Object.keys(settings.blockchain_specific).forEach(function(key, index, map) {
          const sApis = settings.blockchain_specific[key]
          if (sApis.enabled == true && sApis['public_apis'] != null && sApis['public_apis'][method] != null) {
            method_enabled = sApis['public_apis'][method].enabled
          }
        })
      }
    ...
    function specialApiCase(method_name, net=settings.getDefaultNet(), query_parameters) {
      const rpcName = settings.api_cmds[method_name]
      var params = []
      var cmd
      switch (method_name) {
        case 'getnetworkhashps':
        ...
        case 'getnextrewardwhenstr':
        case 'getsupply':
          cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds[method_name])
          method_name = cmd.method
          params = cmd.parameters
          break
        case 'getblockhash':
          ...
          cmd = prepareRpcCommand(rpcName, params)
          method_name = cmd.method
          params = cmd.parameters
          break
        case 'getblock':
        ...
        case 'verifymessage':
        case 'validateaddress':
          cmd = prepareRpcCommand(rpcName, query_parameters)
          method_name = cmd.method
          params = cmd.parameters
          break
        case 'sendmany':
  ...
  function setAccess(type, access_list) {
    ...
    if (type == 'default-safe') {
      ...
      for (i = 0; i < RESTRICT_LIST_DEFAULT_SAFE.length; i++)
        accesslist[RESTRICT_LIST_DEFAULT_SAFE[i]] = false
    }

    if (type == 'read-only') {
      accesslist.type = 'restrict'
      for (i = 0; i < RESTRICT_LIST_READ_ONLY.length; i++)
        accesslist[RESTRICT_LIST_READ_ONLY[i]] = false
    }
  }
