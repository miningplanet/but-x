            ...
            if (err) {
              ...
              if (typeof response === 'object') {
                if (method == 'getdifficulty') {
                  ...
                  rpcCache.set(net + '-' + method, obj);
                  debug("Cached '%s' '%s' %o - mem: %o", method, net, obj, process.memoryUsage());
                  res.json(obj);
                } else {
                  rpcCache.set(net + '-' + method, response);
                  debug("Cached '%s' '%s' %o - mem: %o", method, net, response, process.memoryUsage());
                  res.json(response);
                }
              } else {
                res.setHeader('content-type', 'text/plain');
                rpcCache.set(net + '-' + method, response.toString(), process.memoryUsage());
                debug("Cached '%s' '%s' %o - mem: %o", method, net, response);
